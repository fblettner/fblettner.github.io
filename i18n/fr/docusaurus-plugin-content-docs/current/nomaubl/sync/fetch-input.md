...
| **Output types** | Les sorties de job à récupérer lors de l'extraction des sorties — `PDF`, `XML`, `Excel`… Plusieurs types peuvent être combinés (par ex. le XML plus le PDF lisible à intégrer dans l'UBL). Un défaut s'applique à tous les états et chaque filtre d'état peut le surcharger ; vide = toutes les sorties. |
