...
:::info[Le PDF lisible affiche les libellés, pas les codes]
...
| **Zéros inutiles** | Retire les décimales à zéro des montants, prix unitaires et quantités : un montant entier s'affiche `23` au lieu de `23,00`, et un prix unitaire `23,0000` devient `23`. Désactivé par défaut — l'affichage reste à deux décimales pour les montants et quatre pour les prix. Le réglage est par modèle et s'applique au tableau des lignes, au bloc des totaux et aux PDF lisible et à l'écran. |
