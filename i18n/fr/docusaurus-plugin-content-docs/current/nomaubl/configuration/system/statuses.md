...
- un indicateur **Collect** déterminant l'interrogation du statut depuis la PA.
...
| **Label FR** | `Déposée` | Libellé français affiché dans l'interface lorsque la locale active est le français. |
| **Label EN** | `Deposited` | Libellé anglais affiché dans l'interface lorsque la locale active est l'anglais. |
...
| **Collect** | case à cocher | Lorsque cochée, NomaUBL **interroge ce statut depuis l'API de la PA** (durant le cycle de récupération des statuts de cycle de vie configuré sous *E-Invoicing → PA Connection → Status Retrieval*). À décocher pour exclure un statut du polling. |

Utiliser le bouton **+ Add status** pour ajouter une ligne et le bouton **×** pour en supprimer une.

---

## Articulation des champs entre eux

- Le **Code** constitue la source de vérité en base de données.
...
- **Label FR / Label EN** alimentent les textes de l'interface — l'ajout d'une nouvelle locale nécessite une extension du schéma, et non un simple ajout textuel ici.
...
- **Aligner les Codes sur la réglementation.** Ils constituent le langage commun entre NomaUBL, la PA et tout outillage aval — ne pas inventer de Codes personnalisés.
- **Considérer les Tags comme un contrat interne stable.** Tout renommage d'un Tag implique également la mise à jour de la constante `InvoiceStatusCatalog` correspondante dans le code Java, sous peine d'échecs de résolution à l'exécution.
...
- **Les libellés bilingues ne sont pas optionnels.** Renseigner systématiquement Label FR et Label EN — lorsque le libellé de la locale active est vide, l'interface se replie sur le Code, ce qui rend la lecture difficile.
