...
| **Invoice Lines** | BT-126 à BT-161 | Lignes de facture, avec le pays d'origine (BT-159), le code de classification (BT-158, groupe répétable jusqu'à quatre codes), la version de schéma (BT-158-2), l'**identifiant article normalisé** (BT-157, `cac:Item/cac:StandardItemIdentification/cbc:ID`) — un code enregistré comme un GTIN, sa valeur depuis un champ source et son schéma choisi dans la liste ISO 6523 (par ex. `0160` pour un GTIN), et le **prix brut** (`TAG_LINE_GROSS_PRICE` → BT-148) avec la **quantité de base** (`TAG_LINE_PRICE_BASE_QTY` → BT-149). Un spool portant brut + remise mappe les deux et le prix net est déduit (brut − remise) ; un spool portant net + remise émet son brut égal à net + remise. Les champs de ligne sont regroupés par thème (identification, quantités, prix, TVA, références, classification) — voir [scoping](#scoping) ci-dessous. |
...
- **Chemins d'éléments dans les conditions.** Dans les définitions à séparateur « | » des champs d'extension personnalisés, des notes et des attributs d'article, les colonnes **condition** et **non-vide** acceptent un chemin avec « / » — par ex. `Facture_Entete_S4/CLI_TYPE_ID242` — en plus d'un simple nom d'élément, nécessaire quand le champ source se trouve sous un groupe intermédiaire. Un nom simple se comporte comme avant : les modèles existants ne changent pas ; la colonne valeur acceptait déjà les chemins.
