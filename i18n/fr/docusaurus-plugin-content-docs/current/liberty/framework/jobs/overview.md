...
- La liste des étapes qui se remplit en temps réel à mesure que chaque étape transite par `running` → `succeeded` / `failed`.
...
- [Apps et Plugins → Plugins](../apps/plugins.md) — écrire l'appel Python derrière une étape `python`.
