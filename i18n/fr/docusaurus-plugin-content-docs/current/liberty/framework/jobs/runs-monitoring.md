...
description: "Où observer l'exécution d'une tâche Nomaflow : la page Paramètres → Tâches, le tableau d'historique d'exécution, le tiroir de détail avec la chronologie par étape, la fin de log en direct diffusée via Socket.IO, le parcours d'abandon et la politique de rétention."
...
Chaque exécution Nomaflow laisse une trace : une ligne dans `ly2_job_runs`, une ligne par étape dans `ly2_step_runs`, chaque ligne de log dans `ly2_job_logs`. La page *Paramètres → Tâches* est l'endroit où cette trace est parcourue, filtrée et diffusée en direct.

Cette page décrit le tableau d'historique d'exécution, le tiroir de détail par exécution, la fin de log en direct, le parcours d'abandon et la politique de rétention.
...
<div style={{border: '1px solid rgba(255,255,255,0.10)', borderRadius: '10px', overflow: 'hidden', margin: '20px 0', background: 'rgba(255,255,255,0.02)', fontSize: '12px'}}>
  <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 14px', borderBottom: '1px solid rgba(255,255,255,0.08)', background: 'rgba(255,255,255,0.03)'}}>
    <div style={{fontWeight: 700}}>Exécutions de tâches · 7 derniers jours</div>
    <div style={{display: 'flex', gap: '6px'}}>
      <span style={{padding: '4px 10px', borderRadius: '6px', border: '1px solid rgba(255,255,255,0.15)', fontSize: '11px'}}>Tâche ▾</span>
      <span style={{padding: '4px 10px', borderRadius: '6px', border: '1px solid rgba(255,255,255,0.15)', fontSize: '11px'}}>Statut ▾</span>
      <span style={{padding: '4px 10px', borderRadius: '6px', border: '1px solid rgba(255,255,255,0.15)', fontSize: '11px'}}>Recharger</span>
    </div>
  </div>
  <div style={{display: 'grid', gridTemplateColumns: '90px 1fr 100px 110px 100px 80px', padding: '10px 14px', textTransform: 'uppercase', letterSpacing: '0.06em', opacity: 0.7, borderBottom: '1px solid rgba(255,255,255,0.08)', fontSize: '11px', fontWeight: 600}}>
    <div>Id d'exécution</div><div>Tâche</div><div>Statut</div><div>Démarré</div><div>Durée</div><div>Déclencheur</div>
  </div>
  <div style={{display: 'grid', gridTemplateColumns: '90px 1fr 100px 110px 100px 80px', padding: '10px 14px', borderBottom: '1px solid rgba(255,255,255,0.05)', alignItems: 'center'}}>
    ...
    <div>invoices-nightly-rebuild</div>
  ...
  <div style={{display: 'grid', gridTemplateColumns: '90px 1fr 100px 110px 100px 80px', padding: '10px 14px', alignItems: 'center'}}>
    ...
    <div>invoices-monthly-close</div>
...
| **Tâche** | Nom de la tâche. La pastille de filtre en haut restreint à une seule tâche. |
...
- Une liste déroulante **Tâche** — restreindre à une seule tâche.
...
- Un bouton **Recharger** — qui se rafraîchit aussi automatiquement toutes les 5 s tant qu'au moins une exécution est en `running`.
...
| `failed` | Une étape a épuisé ses nouvelles tentatives et a arrêté la tâche. | Oui |
| `partial-success` | Certaines étapes ont échoué avec `continue_on_error = true` ; la tâche a poursuivi. | Oui |
| `aborted` | Un opérateur a cliqué sur *Abandonner* ou le `timeout_seconds` au niveau de la tâche s'est déclenché. | Oui |
| `skipped` | La tâche devait se déclencher mais une dépendance / un verrou d'instance unique l'en a empêché. | Oui |
...
- Id d'exécution, nom de la tâche, pastille de statut, horodatages de démarrage / fin, durée, déclencheur.
- Un bouton **Abandonner** (visible uniquement quand `status = running`, uniquement pour les utilisateurs porteurs de `job:<name>:abort` ou du global `job:*`).
- Un bouton **Rejouer** (visible quand le `status` est final) qui déclenche une nouvelle exécution avec le même instantané de `params`.
...
| Instantané d'**entrée** | La table `params` / `kwargs` après toutes les substitutions `${...}`. Utile quand la même étape s'est exécutée différemment d'une nouvelle tentative à l'autre. |
...
### Fin de log en direct

Un panneau défilant sous la chronologie diffuse chaque ligne de log de l'exécution — à la fois les événements structurés du framework (`step started`, `step finished`, `retry triggered`) et les messages émis par les appels d'étape `python` via `logging.getLogger(__name__)`.
...
- Le nom du logger (par exemple `invoices.invoicing`).
- Le message.
...
5. La tâche est enregistrée avec `status = aborted` et aucune étape suivante ne s'exécute.
...
Le bouton **Rejouer** sur une exécution terminée crée une nouvelle exécution avec :

- Le même instantané de `params`.
- Un nouveau `run_id`.
- `triggered_by = "user:<operator>"` et `replay_of = <original run_id>`.

Le lien `replay_of` est visible dans le tiroir de détail sous la forme « Rejeu de r-1840 ». Utile pour les tâches nocturnes qui ont échoué à cause d'un incident passager et qui ont besoin d'une nouvelle tentative sans attendre le prochain déclenchement cron.
...
| `POST /admin/jobs/runs/<run_id>/replay` | Rejouer avec les mêmes params. |
| `GET  /admin/jobs/runs/<run_id>/logs?level=&follow=` | Diffuser la fin de log. `follow=true` bascule sur Socket.IO. |

Chaque endpoint demande la permission `job:<name>` pour la tâche ciblée (ou `job:*`). L'endpoint de liste ne retourne que les exécutions des tâches que l'appelant peut voir — filtré automatiquement.

---

## Rétention

Les lignes d'exécution et les lignes de log sont purgées par une tâche interne (`_default/cleanup-job-history`) qui se déclenche une fois par jour à 03:00. La politique de rétention :
...
Pour une piste d'audit à long terme, router le flux de log vers l'agrégateur de logs central (Loki, Splunk, Datadog) via `LIBERTY_LOG_JSON=1` et considérer l'historique interne du framework comme un cache opérationnel.
...
| Panneau | Contenu |
|---|---|
| **Exécutions en cours** | Chaque tâche en `status = running`, avec le temps écoulé et l'étape courante. Cliquer sur une ligne ouvre le tiroir de détail. |
| **Échecs récents** | Les 20 dernières exécutions `failed` / `aborted`. |
| **Pouls de l'ordonnanceur** | Dernier déclenchement, prochain déclenchement, profondeur de la file. Utile pour confirmer que l'ordonnanceur reste actif un jour calme. |
| **Statistiques de pool** | Connexions ouvertes / inactives / utilisées par pool — fait apparaître une saturation de pool qui bloquerait sinon les tâches. |

Ce tableau de bord est le premier endroit où regarder quand « une tâche ne s'est pas exécutée » — le pouls de l'ordonnanceur indique si le framework a même essayé.
...
- **Utiliser `log.warning()` pour « ok mais inhabituel ».** Une ligne qui prend 10× plus de temps que d'habitude mérite un WARNING dans le log, pas du silence. Le filtre de niveau dans la fin de log les rend faciles à rechercher.
- **Marquer l'exécution avec un identifiant porteur de sens.** Une étape `python` qui logge `log.info("invoices.invoicing.run period=2026-05 drafts=42 dry_run=False")` rend l'historique d'exécution facile à rechercher.
- **Ne pas garder l'unique historique dans le framework.** Transférer le log JSON vers l'agrégateur — les incidents côté installation ne devraient pas emporter avec eux la piste d'audit.
...
- [`jobs.toml`](./jobs-toml.md) — la déclaration de tâche qui pilote chaque exécution.
- [Types d'étape](./step-types.md) — ce que chaque étape enregistre dans son résultat.
- [Référence API REST → Tâches](../rest-api.md#jobs) — le contrat complet des endpoints.
