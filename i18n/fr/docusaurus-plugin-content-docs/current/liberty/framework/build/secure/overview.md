...
| **Clé de licence** | Conditionne les connecteurs marqués `licensed = true` (Nomasx-1, Nomajde, NomaUBL …). | `app.toml` `[license] key` ou la variable d'environnement `LIBERTY_LICENSE_KEY`. |
...
Certains connecteurs portent un drapeau `licensed = true` — ils ne sont chargés que si le framework dispose d'une clé de licence valide. Exemples : Nomasx-1, Nomajde, NomaUBL.
