...
3. Échoue au parsing quand `default` est manquant ou vide.
...
|---|---|---|
| Clé `default` manquante. | Le connecteur échoue au parsing à l'enregistrement / au rechargement. L'erreur mentionne « a per-dialect sql map must include a 'default' key ». | Ajoutez une variante `default`. |
| `default` vide. | Même erreur de parsing — « the 'default' sql variant must not be empty ». | Remplissez-la — même un placeholder comme `SELECT 1` suffit à passer la validation pendant que vous finissez les autres. |
