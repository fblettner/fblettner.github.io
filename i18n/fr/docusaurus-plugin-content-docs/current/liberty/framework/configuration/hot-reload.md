...
description: "Chaque édition de TOML par section faite dans l'UI Paramètres ou poussée via POST /admin/reload est appliquée au framework en cours d'exécution sans redémarrage. Cette page documente ce qui se recharge, ce qui reste en cours et ce qui demande vraiment un redémarrage du processus."
...
Cette page est le contrat : ce qui se recharge, ce qui ne le fait pas, ce qui arrive aux requêtes en cours, et ce qui demande vraiment un redémarrage du processus.
...
<div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '14px', margin: '24px 0'}}>
  ...
  <div style={{border: '1px solid rgba(255,159,10,0.40)', borderRadius: '10px', padding: '16px', background: 'rgba(255,159,10,0.04)'}}>
    ...
    <div style={{fontSize: '12px', lineHeight: '1.55'}}>changement d'URL de pool — anciennes connexions drainées au fur et à mesure que les requêtes en cours se terminent, les nouvelles utilisent l'URL mise à jour</div>
  </div>
...
2. **Construire le nouveau registre à côté.** Chaque entrée est validée contre son modèle Pydantic ; une entrée invalide est rapportée avec son nom et son message. Une seconde passe vérifie les références (un écran pointant vers un connecteur inexistant, une feuille de menu pointant vers un écran inexistant) et refuse le rechargement quand quelque chose est cassé.
3. **Permuter le registre de manière atomique.** Le nouveau registre remplace l'ancien à l'intérieur d'un même verrou. Les requêtes qui ont déjà démarré contre l'ancien registre **se terminent dessus** ; les nouvelles requêtes prennent le nouveau. Il n'y a aucun état intermédiaire où une partie de la requête voit l'ancienne configuration et une autre partie voit la nouvelle.
...
|---|---|
| **Requête SQL** | La requête se termine contre la définition de connecteur qui était courante à son arrivée. Une re-récupération depuis le même onglet du navigateur utilise la nouvelle. |
...
| **Rotation du secret JWT** | Un nouveau secret invaliderait tous les tokens existants en plein vol. | Redémarrage roulant — définissez le nouveau secret sur chaque réplica à tour de rôle pour que les sessions utilisateurs migrent en douceur. |
...
| **Échange de clé de licence** | Les barrières de fonctionnalité sont évaluées au démarrage contre le payload du JWT. | Redémarrer. |
...
| **Référence manquante** | Une feuille de menu pointe vers un écran qui vient d'être renommé ou supprimé. | Le rechargement avorte en phase 2 ; le diagnostic nomme la feuille de menu et l'écran manquant. Utilisez *Renommer* dans les éditeurs Connecteurs / Écrans / Dictionnaire plutôt que d'éditer les identifiants à la main — il propage le changement à travers chaque fichier référent. |
...
| **Base de données inaccessible** | Une nouvelle URL de pool pointe vers un hôte hors service. | Le pool est créé paresseusement, donc le rechargement lui-même réussit ; le premier appel SQL contre le nouveau pool échoue avec l'erreur de connexion. Le bouton *Tester* de l'UI Paramètres attrape cela plus tôt. |
| **TOML vide** | Un `Enregistrer` a effacé toutes les entrées. | Le rechargement réussit — le registre a zéro entrée. Chaque surface dépendante (catalogue Connecteurs, menu) devient vide. Restaurez depuis le commit git précédent. |

---

## Conseils et bonnes pratiques

- **Éditez dans l'UI Paramètres, pas sur disque.** L'UI exécute le même pipeline de validation avant d'écrire le fichier, donc un enregistrement via éditeur ne peut jamais produire un fichier que le rechargement rejetterait.
- **Utilisez *Renommer* à travers les fichiers.** Renommer un connecteur ou un écran depuis l'éditeur réécrit chaque référence en une seule transaction ; renommer à la main en TOML risque une référence orpheline.
- **Auditez le log de rechargement.** Chaque rechargement réussi écrit une ligne INFO par registre avec le nombre d'entrées ; une chute soudaine du compte est le signal le plus précoce qu'une édition d'opérateur a supprimé plus que prévu.
- **Gardez `watch_config` désactivé en production.** Une veille de fichiers qui bat (`rsync`, `git pull` en milieu de journée) peut déclencher un rechargement alors que seulement la moitié des fichiers est à jour.
- **Redémarrez sur les éditions de `app.toml`.** Une rotation JWT qui échoue est plus visible qu'une valeur obsolète chargée silencieusement.
