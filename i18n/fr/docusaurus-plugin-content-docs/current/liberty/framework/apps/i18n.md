...
La réponse est mise en cache côté serveur et étiquetée par la révision du bundle i18n — les clients ne re-récupèrent que quand la révision change (après un `POST /admin/reload` sur la portée i18n).
...
- [Référence REST API](../../references/rest-api.md) — `GET /api/i18n/<lang>` et le contrat de l'en-tête `X-Liberty-Lang`.
