...
Les filtres de la barre d'outils restreignent la liste par *App* et par *Type*. **+ Nouveau connecteur** ouvre l'éditeur sur une entrée vierge ; cliquer sur une ligne ouvre l'éditeur sur ce connecteur.
...
| **hors ligne** | La sonde a échoué. Le connecteur reste affiché, mais toute requête contre lui échouera tant que la source ne sera pas revenue. |
...
<div style={{border: '1px solid rgba(255,255,255,0.10)', borderRadius: '10px', overflow: 'hidden', margin: '20px 0', background: 'rgba(255,255,255,0.02)', fontSize: '12px'}}>
  ...
  <div style={{padding: '14px 16px'}}>
    ...
    <div style={{display: 'grid', gridTemplateColumns: '1.6fr 70px 80px 40px', rowGap: '4px', alignItems: 'center', fontSize: '11px'}}>
      ...
      <div>refresh-totals</div><div><span style={{padding: '2px 8px', borderRadius: '999px', background: 'rgba(255,159,10,0.10)', border: '1px solid rgba(255,159,10,0.40)', color: '#fb923c', fontSize: '10px', fontWeight: 600}}>écriture</span></div><div style={{fontFamily: 'ui-monospace, monospace', opacity: 0.6}}>1 param</div><div style={{textAlign: 'right', opacity: 0.55}}>✏️</div>
    </div>
  </div>
</div>
...
| **App** | Liste déroulante des apps déclarées. Détermine l'espace de noms du connecteur et l'espace de travail où il apparaît. |
...
| **Indices de colonnes** | Sous-table optionnelle. Chaque ligne lie une colonne du résultat à une entrée du dictionnaire : *Colonne* (liste déroulante des colonnes découvertes par le bouton *Tester*) + *Dictionnaire* (liste déroulante des entrées du dictionnaire). Détermine libellés, formats, énumérations, lookups. Les colonnes sans indice retombent sur le nom de la colonne comme libellé. |

Le bouton **▶ Tester** en haut de l'éditeur exécute la requête contre le pool live avec des valeurs de remplissage pour les paramètres et affiche les 50 premières lignes. Le premier test réussi remplit la liste déroulante *Indices de colonnes* avec les colonnes découvertes ; les tests suivants la rafraîchissent.
...
| **Authentification** | `Aucune` / `Basic` / `Bearer` / `OAuth2`. Chaque choix ouvre un sous-formulaire. *Basic* demande utilisateur + 🔒 mot de passe ; *Bearer* prend un 🔒 jeton ; *OAuth2* demande l'endpoint de jeton, le client id, le 🔒 client secret et la liste de scopes. |
...
`API` est un type hybride conservé pour la compatibilité avec les installations qui aiguillent certains appels vers SQL et d'autres vers HTTP sous un même nom de connecteur. L'éditeur expose à la fois les sous-tables *Requêtes* et *Endpoints* ; le champ *Opération* sur chaque entrée choisit le moteur sous-jacent.

Sur une nouvelle installation, on choisira presque toujours `SQL` ou `HTTP` directement. `API` est la voie quand il faut consolider plusieurs connecteurs hérités sous un même chapeau.
...
La sous-table **Indices de colonnes** est l'endroit où surcharger les valeurs par défaut. Un indice associe une colonne découverte à une entrée de [dictionnaire](./dictionary.md) — c'est ainsi que la colonne reçoit un libellé localisé, une règle boolean / enum / lookup, un format numérique, etc.
...
- **User les indices de colonnes avec parcimonie.** Une colonne dont le nom de base suffit n'a pas besoin d'indice — définir des indices uniquement quand le dictionnaire apporte une valeur ajoutée (libellé localisé, format, lookup).
...
Les définitions de connecteurs sont enregistrées dans `liberty-apps/config/connectors.toml` (ou `liberty-next/config/connectors.toml` quand `LIBERTY_APPS_DIR` n'est pas défini). Les opérateurs **ne modifient pas ce fichier à la main** en exploitation normale ; l'éditeur de connecteurs est l'interface canonique. L'onglet *TOML brut* de [Paramètres → Connecteurs](./configuration/settings-ui.md) est l'échappatoire pour les rares cas où l'éditeur ne couvre pas un champ.
...
- [Dictionnaire](./dictionary.md) — métadonnées par colonne : libellés, formats, énumérations, lookups.
...
- [Chiffrement et secrets](./configuration/encryption-secrets.md) — la bascule 🔒 sur les mots de passe / jetons.
