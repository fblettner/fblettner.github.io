...
- **E-Directory: Peppol registration and platform per company.** Expanding a company card now also queries the public Peppol network (no account needed): a new *Peppol directory* section lists each registered participant (French registrations use ICD 0225, including per-establishment identifiers), its name, registration date and — resolved through the official SML/SMP discovery — the **service provider operating it** (the PA, e.g. Pennylane, with its Peppol seat id; access-point and SMP URLs in the tooltip). The lookup runs on first expand only, and is also exposed as `POST /api/peppol-lookup` for integrations (documented in the API reference).
...
- **E-Directory: every PPF addressing identifier visible.** The page is reorganised by company: one collapsed card per SIREN (name, head-office address, INSEE state, directory summary) that expands into the INSEE establishments (their own collapsible group) and a new **PPF directory lines** list — every identifier registered for the SIREN, whatever its form (SIREN, SIREN+SIRET, routing code, suffix), with its enabled/disabled state. Identifiers like `422250845_FGX`, previously undetectable, now appear. One directory call per company replaces the per-row probes; searching by SIREN or SIRET now lists all establishments (a second INSEE lookup by company name fills what the numeric search omits). The listing goes through the connector's `directory-check-siren` endpoint, now returning the full lines with per-endpoint response mappings (ATGP and Yooz shapes supported); when the endpoint is not configured, the page says so instead of guessing. The processing-time directory check is unchanged; the `directory-check-siret` endpoint is obsolete.
