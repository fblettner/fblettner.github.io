...
:::info[The readable PDF shows labels, not codes]
...
- **Line Table** — group-header toggles (*Delivery group*, *Page break per delivery*, *Document Reference group* and,, *Group by customer order* — a third band below the delivery and document-reference bands, showing the referenced customer-order ID, BT-132), seven column toggles (`Line #`, `Description`, `Quantity`, `Unit`, `Unit Price`, `Amount`, `Tax`) and nine sub-detail toggles for line metadata (BT-127, BT-134/135, **BT-155 — Seller's item code**, BT-156, BT-157, BT-158, **BT-159 — Country of origin** *(prints the country's full name resolved from the shared countries list — not the ISO code)*, allowances / charges, additional item properties). The customer-order band follows the same reset rules as the others: a new delivery re-prints both inner breadcrumbs, a new document reference re-prints the customer-order band.
...
| **Trim zeros** | Drops trailing zero decimals from amounts, unit prices and quantities — a whole amount prints as `23` instead of `23.00`, and a `23.0000` unit price prints as `23`. Off by default, which keeps the fixed two-decimal amount and four-decimal price display. The setting is per template and applies to the line table, the totals box, and both the readable and on-screen PDFs. |
