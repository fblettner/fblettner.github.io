...
| **PA Code(s)** | `fr_e_invoicing_200` | Event name sent to the Plateforme Agréée API — placed in the `names[]` payload of status calls. **Several codes, comma-separated**, may map to this one status, so a single list serves invoices whose statuses arrive from different sources (your main platform and Chorus Pro, say) without duplicating it. Each code must match what its PA expects exactly. |
