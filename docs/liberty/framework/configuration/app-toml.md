...
For ops scripts and CI pipelines that need to drive the framework without the UI, the same surface is reachable via the REST endpoints under `/admin/config/framework/*` — see [REST API reference](../rest-api.md#admin-config).
