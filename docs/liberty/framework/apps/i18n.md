...
:::info[Deep reference]
This page documents the i18n resolution chain, the language pack format, per-app overrides and RTL handling. For task-oriented translation of a menu (`l.fr`, `l.de` and Lucide icons), see [Build → Menus → Translations and icons](../build/menus/translations-and-icons.md).
:::
...
- [Apps](./overview.md) — workspace selector that uses the `apps` translation namespace.
- [Concepts → Dictionary](../dictionary.md) — where inline `label = { … }` maps are declared.
- [REST API reference](../../references/rest-api.md) — `GET /api/i18n/<lang>` and the `X-Liberty-Lang` header contract.
